...
def run(cmd: list[str], cwd: Path | None = None, show: int = 12) -> tuple[int, str]:
    ...
    try:
        p = subprocess.run(
            cmd, cwd=str(cwd) if cwd else None, capture_output=True, text=True, timeout=120
        )
...
def friction(
    n: int, what: str, expected: str, actual: str, fix: str, kind: str = "friction"
) -> None:
    LOG.append(
        {"beat": n, "kind": kind, "what": what, "expected": expected, "actual": actual, "fix": fix}
    )
...
def lay_the_table(play: Path) -> Path:
    ...
    for name, taken in PHOTOS:
        ...
        side = shed / (name + ".json")
        side.write_text(json.dumps({"file": name, "taken": taken, "origin": ORIGIN}, indent=2))
    return shed
...
def b1_the_ask() -> None:
    ...
    lines = [
        f"keywords found : {' · '.join(dict.fromkeys(h.matched for h in hits)) or '(none)'}",
        "candidate majors:",
    ]
    ...
    lines.append(
        f"{len(ask)} candidates, no way to choose"
        if ask
        else (f"one major: {next(iter(by_major))}" if by_major else "no keyword matched")
    )
    shows("\n".join(lines))
    ...
    try:
        ...
        friction(
            1,
            "the entry refused: Nestor is not installed",
...
def b2_look_in_the_box() -> None:
    ...
    for app, why in [
        ("source-trail", "file_read, pattern_storage — follows a file's provenance"),
    ...
    says(
        "The box",
        "You may not need to build this. source-trail already reads a "
...
def b3_she_builds_anyway() -> None:
    ...
    friction(
        3,
        "every warrant on the box is an attestation",
...
def b4_the_playground(play: Path) -> Path:
    ...
    keeping = {
        "app_id": "rally-dates",
        "author": "fixture:rosalind",
        "made": str(date.today()),
        "for": "list the photographs with no date",
        "pins": [],
        "origin": ORIGIN,
    }
    ...
    if not real.exists():
        friction(
            4,
            "the playground does not exist on this box",
...
def b6_it_wants_the_web() -> None:
    ...
    says(
        "The box",
        "I cannot reach the internet. Two of the three keys are missing, "
        "and one of them is yours to turn — for this one task, expiring, "
        "bound to this exact job.",
    )
    friction(
        6,
        "a denial does not say which key is missing",
    ...
    friction(
        6,
        "the per-task envelope has never been exercised",
...
def b7_the_review(proj: Path) -> None:
    ...
    rationale = (
        "It duplicates source-trail but returns a list instead of a lookup, "
    ...
    if has_stance:
        ...
        friction(
            7,
            "the stance-aware scorer is not present",
            ...
            "re-sync the vendored file from willow-mcp",
            kind="missing",
        )
    pause("beat")
    says("Loki", "SILENCE.")
...
def b8_the_bar(proj: Path) -> None:
    ...
    if check.exists():
        ...
        friction(
            8,
            "promote_check.py not found where the README says it is",
            "the bar is executable",
            f"no {check.relative_to(GH)} on this box",
    ...
    says(
        "The box",
        "It does not clear the bar. It needs tests, a manifest, and a "
        "demo. The demo is the one almost everybody misses — 39 of the "
        "42 apps in the store do not have one.",
    )
    friction(
        8,
        "the bar has never been pointed at a playground",
        ...
        "point promote_check at the playground once it exists",
        kind="missing",
    )
...
def b9_the_demo() -> None:
    ...
    """)
    note(
        f"recorder present: {'yes' if rec.exists() else 'no'} — "
...
def b10_the_seal() -> None:
    beat(
        10,
        "The seal",
        ("Every gate has passed. The last one is a person, and she is not at her desk."),
    )
    shows("""
    ...
    friction(
        10,
        "the seal happens somewhere else",
    ...
    friction(
        10,
        "the phone seat does not exist yet",
...
def b11_calibration() -> None:
    ...
    try:
        ...
        friction(
            11,
            "the build loop could not run",
            "the box states a confidence, she answers, the box learns",
            f"{type(err).__name__}: {err}",
            "pip install nestor-meaning — the loop seals through memory",
            kind="missing",
        )
    else:
        ...
        if not outcome.sealed:
            friction(
                11,
                "the decision was made but not sealed",
...
def report(as_json: bool) -> int:
    ...
    if as_json:
        print(
            json.dumps(
                {"origin": ORIGIN, "missing": missing, "friction": rough, "facts": FACTS},
                indent=2,
                default=str,
            )
        )
