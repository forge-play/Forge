...
from forge import checkpoint_memory, entry, store_diff
...
def _seal(store, q, a, verifier="rosalind"):
    ...
    return memory.add_pair(
        q, a, "decision", "decision", status="sealed", verifier=verifier, store=store
    )
...
def stores(tmp_path):
    ...
    new = pm.propose(
        "Where is the project Nestor? (moved)", "~/.forge/projects/<id>/nestor", origin="t"
    )
...
def test_a_question_retired_by_revision_is_retired_not_a_conflict(tmp_path):
    ...
    memory.revise_draft(
        q, "~/.forge/projects/<id>/nestor", "decision", "decision", reason="moved", store=proj
    )
...
def test_the_four_sets(stores):
    ...
    assert {x["question"] for x in d.proposed_here} == {
        "Does the first bite get a keyword row?",
    ...
    assert (
        d.conflicts[0]["here"]["answer"] == "gh first"
        and d.conflicts[0]["there"]["answer"] == "the bot's inbox first"
    )
    ...
    assert s.startswith(
        "behind 1 (sealed upstream), ahead 2 (proposed here), 1 conflict(s), 1 retired here but sealed there; 1 agreed"
    )
...
def test_main_as_a_bundle_reads_the_same(stores, tmp_path):
    ...
    for d in (d_dict, d_path):
        assert (
            d.behind == 1
            and d.ahead == 2
            and len(d.conflicts) == 1
            and len(d.retired_here_sealed_there) == 1
        )
    assert d_dict.main_source.startswith("bundle (digest") and d_path.main_source.startswith(
        "bundle main-bundle.json"
    )
...
def test_diff_writes_nothing(stores, tmp_path):
    ...
    assert (
        before == after
        and len(proj.memory_list(limit=1000)) == n_p
        and len(main.memory_list(limit=1000)) == n_m
    )
...
def test_a_missing_main_path_refuses_and_creates_nothing(stores, tmp_path):
    ...
    assert not missing.exists() and not missing.parent.exists(), (
        "a write-free verb created a database"
    )
...
def test_the_entry_reports_the_main_diff_or_says_it_was_not_consulted(tmp_path, monkeypatch):
    ...
    e4 = entry.open_bite(
        ...
        responder=_Responder(),
        root=tmp_path / "cp",
        main=tmp_path / "nope.db",
    )
    assert (
        e4.tiers["main"].startswith("could not read main") and not (tmp_path / "nope.db").exists()
